...
%% When we select the first 8 patients only: These patients are chosen as 
% training data and the rest are for test/evaluation as they are missing many data poinnts

% % Loop over the first 8 patients (choosen as training datasets)
% for i = 1:8
%     % Get the patient number from the sorted list
%     patient_num = sorted_patientstimepoints(i, 1);
% 
...
% 
% 
% for i = 1:8 % 8 patients are choosen for training datasets the rest will be used only for testing 
%     for j = 2:size(sorted_patientstimepoints,2)
%         if sorted_patientstimepoints(i,j) == 0
%             [indx,~] = find(sorted_patientstimepoints(1:8,j));
%             meanData = sumData(:,j-1) / length(indx);
